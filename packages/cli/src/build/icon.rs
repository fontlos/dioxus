...
use std::{
    fs::{create_dir_all, write, copy, File},
    io::{BufWriter, Write},
    path::{Path, PathBuf},
};
...
fn copy_android_vector_drawable_xml(path: &Path,out_dir: &Path) -> Result<()> {
    ...
    copy(
        path,
        out_dir.join("drawable-v24").join("ic_launcher_foreground.xml"),
    )?;
    Ok(())
}
...
pub fn gen_android_icons(icon: &[String], out_dir: &Path) -> Result<()> {
    ...
    for p in icon.iter() {
            ...
            .unwrap_or(false)
        {
            ...
            tracing::info!("Found Android VectorDrawable XML icon: {}. Copy finished", p);
            return Ok(());
        }
    }
    ...
    if source.is_none() {
        for p in icon.iter() {
            let path = Path::new(p);
            if path.exists() {
                source = Some(Source::DynamicImage(open(path).context("Failed to open image")?));
                break;
            }
        }
    }
